...
contract FullDaoContract {
    ...
    // proper syntax conventions on mappings @rb
    //should be mapping(address  => uint256) MsigToPhoneNumber;
    ...
    function createDao(
        ...
        uint256 _multiSigPhoneNo // bytes memory _daoIdentifier
    ) public {
     ...
     */
    function isPhoneNumberMappedToMsig(
        uint256 _phoneNumber
     ...
     */
    function getMemberByPhoneNumber(
        address _daoMultiSigAddr,
        uint256 _phoneNumber
    ) public payable returns (MemberDetails memory) {
    ...
    //You could use an event emission to sigmify success @rb
    function addProposal(
        ...
        bool isUserDaoMember = isDaoMember(
            address(msg.sender),
            _daoMultiSigAddr
        );
        require(
            isUserDaoMember != true,
            "YouHaveToBeADaoMemberToCreateProposal"
        ); //Redirect to request to join dao for which the multisig they were trying
     ...
     */
    function isProposalExpiredByTitle(
        address _daoMultiSigAddr,
        string memory _pTitle
    ) public view returns (bool) {
        ...
        for (uint256 i = 0; i < proposals.length; i++) {
            if (
                keccak256(abi.encodePacked(proposals[i].pTitle)) ==
                keccak256(abi.encodePacked(_pTitle))
            ) {
    ...
    function isProposalExpired(
        ...
        require(
            index < daoProposals[_daoMultiSigAddr].length,
            "Index out of bounds"
        );
        ProposalDetails storage proposal = daoProposals[_daoMultiSigAddr][
            index
        ];
        if (
            proposal.expirationTime < block.timestamp + proposal.expirationTime
        ) {
            return false;
        }
    ...
    function castVote(
        ...
        require(
            isDaoMember(msg.sender, _daoMultiSigAddr),
            "Only DAO members can vote"
        );
        require(
            !isProposalExpiredByTitle(_daoMultiSigAddr, _pTitle),
            "Proposal is expired"
        );
     ...
     *         If the proposal is not found, it reverts with "Proposal with the specified title not found".
     */
    function findProposalOwnerByTitle(
        address _daoMultiSigAddr,
        string memory _pTitle
    ...
    function getVotes(
        address _proposalOwner
    ) public view returns (VoteDetails[] memory) {
