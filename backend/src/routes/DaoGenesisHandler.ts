...
import { CreateDaoWithMembers, GetAllDaosInPlatform } from '../controller/DaoController';
...
router.post('/CreateDao', Authenticator, async (req: Request, res: Response) =>{await CreateDaoWithMembers(req, res)});//INFO: Unique route that is only used for creating Daos
